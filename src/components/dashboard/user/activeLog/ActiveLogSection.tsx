...
const PLATFORM_META: Record<ActiveLogEntry["platform"], { label: string; color: string }> = {
...
export function ActiveLogSection() {
  ...
  const filtered = React.useMemo(() => {
    return MOCK_ACTIVE_LOG.filter((entry) => {
      const matchesPlatform = activeFilter === "all" || entry.platform === activeFilter
    ...
    <section className="mx-auto w-full max-w-5xl space-y-8 px-4 py-8 sm:px-6 lg:px-8">
      <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
        <div className="space-y-3">
          ...
          <FilterButtons options={FILTER_OPTIONS} paramName="platform" defaultValue="all" />
        </div>
